var gReportController = {
  ...
  setShootDayPicker: function (picker, date) {
    ...
    if (date instanceof Components.interfaces.calIDateTime) { 
      caldate = date;
      jsdate = date.clone().jsDate;
    }
    else {
      jsdate = date;
      caldate = createDateTime();
      caldate.timezone = calendarDefaultTimezone();
      caldate.year = jsdate.getFullYear();
      caldate.month = jsdate.getMonth();
      caldate.day = jsdate.getDate();
      caldate.isDate = true;
    }
...
try {
    ...
    var seldate = null;
    if (picker.date) {
      seldate = createDateTime();
      seldate.timezone = calendarDefaultTimezone();
      seldate.year = picker.date.getFullYear();
      seldate.month = picker.date.getMonth();
      seldate.day = picker.date.getDate();
      seldate.isDate = true;
    }
...
catch (ex) { dump("*** editShootDayPicker: " + ex + "\n"); }
  },
      ...
      case "strip":
        ...
        if (shootDaysSet && ! document.getElementById("allevents").selected) {
          if (this.startShootPicker.date && this.endShootPicker.date) {
            ...
            start.timezone = calendarDefaultTimezone();
            start.year = jsdate.getFullYear();
            start.month = jsdate.getMonth();
            start.day = jsdate.getDate();
            start.isDate = true;
            jsdate = this.endShootPicker.date;
            end = createDateTime();
            end.timezone = calendarDefaultTimezone();
            end.year = jsdate.getFullYear();
            end.month = jsdate.getMonth();
            end.day = jsdate.getDate();
            end.isDate = true;
          }
        }
        break;
