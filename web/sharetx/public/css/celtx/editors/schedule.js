...
var gScheduleController = {
  ...
  open: function open (project, docres) {
    ...
    try {
      ...
      try {
        ...
        startDate.setHours(Math.floor(startSecs / 3600),
          Math.floor((startSecs % 3600) / 60), startSecs % 60);
  ...
  nextSceneTimeForDate: function nextSceneTimeForDate (date, callback) {
    ...
    var listener = {
      ...
      onOperationComplete: function (aCalendar, aStatus, aOperationType,
                                     aId, aDetail) {
        var daystart = getPrefService().getBranch("celtx.calendar.")
          .getIntPref("daystart"); // in seconds
        var cursor = daydate.clone();
        cursor.resetTo(cursor.year, cursor.month, cursor.day, 0, 0,
          daystart, calendarDefaultTimezone());
