  ...
  sanitizeFragment: function (fragment) {
    ...
    while (curnode) {
      ...
      if (curnode instanceof Components.interfaces.nsIDOMText &&
        ...
        var p = dstdoc.createElement("p");
        p.appendChild(dstdoc.createElement("br"));
        fragment.insertBefore(p, curnode);
        p.appendChild(curnode);
      }
      else if (curnode instanceof IBr)
  ...
  onPasteOrDrop: function onPasteOrDrop (event, trans) {
    if (event != null) {
      ...
      if (this.dragSource != null &&
        ...
        setTimeout(function () {
          gScriptController.clipboardSanitizer.removeIncorrectIDs();
          var scene = getRDFService().GetResource(gPanel.curscene);
          gPanel.extractBreakdownFromScene(scene);
          gPanel.showNotesForScene(scene);
          gPanel.showMediaForScene(scene);
        }, 0);
      }
