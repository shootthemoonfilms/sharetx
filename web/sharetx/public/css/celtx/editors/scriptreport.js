...
var gReportController = {
  ...
  generateDataCallback: function generateDataCallback () {
    ...
    allDS.Assert(allScene, sortordArc, rdfsvc.GetLiteral("0000"), true);
  ...
  createSceneDS: function createSceneDS (doc) {
    var rdfsvc = getRDFService();
    var ds = getInMemoryDataSource();
    var titlearc = rdfsvc.GetResource(Cx.NS_DC + "title");
    var typearc = rdfsvc.GetResource(Cx.NS_RDF + "type");
    var scenetype = rdfsvc.GetResource(Cx.NS_CX + "Scene");
    var ordarc = rdfsvc.GetResource(Cx.NS_CX + "ordinal");

    var xpath = new XPathEvaluator();
    var xset = xpath.evaluate("/script/scene", doc, null,
                   XPathResult.ORDERED_NODE_ITERATOR_TYPE, null);
    var scene = null;
    var i = 1;
    while (scene = xset.iterateNext()) {
      var sceneres = rdfsvc.GetResource(Cx.SCENE_PREFIX +
                                        scene.getAttribute("id"));
      var title = rdfsvc.GetLiteral(scene.getAttribute("title"));
      var ord = rdfsvc.GetIntLiteral(i++);
      ds.Assert(sceneres, typearc, scenetype, true);
      ds.Assert(sceneres, titlearc, title, true);
      ds.Assert(sceneres, ordarc, ord, true);
    }
    return ds;
  },
