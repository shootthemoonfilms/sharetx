var gPDFController = {
  ...
  onScriptLoad: function onScriptLoad () {
    ...
    for (var i = 0; i < metas.length; ++i) {
      if (metas[i].name == "CX.sceneNumbering")
        this.sceneNumbers = metas[i].content;
      else if (metas[i].name == "CX.showCharNumbers")
        this.showCharNumbers = (metas[i].content == "true");
      else if (metas[i].name == "CX.avShowHeader")
        this.showAVHeader = (metas[i].content == "True");
      else if (metas[i].name == "CX.avHeader") {
  ...
  updateMetas: function updateMetas () {
    ...
    for (var i = 0; i < metas.length; ++i) {
      if (metas[i].name == "CX.sceneNumbering")
        cache.sceneNumbering = metas[i];
      else if (metas[i].name == "CX.avShowHeader")
        cache.avShowHeader = metas[i];
    }

    if (! cache.sceneNumbering) {
      cache.sceneNumbering = doc.createElement("meta");
      cache.sceneNumbering.setAttribute("name", "CX.sceneNumbering");
      doc.documentElement.firstChild.appendChild(cache.sceneNumbering);
    }
    cache.sceneNumbering.content = this.sceneNumbers;

    if (gScriptController.mode != "av")
      return;

    // A/V Specific Metas
    if (! cache.avShowHeader) {
      cache.avShowHeader = doc.createElement("meta");
      cache.avShowHeader.setAttribute("name", "CX.avShowHeader");
      doc.documentElement.firstChild.appendChild(cache.avShowHeader);
    }
    cache.avShowHeader.content = this.showAVHeader ? "True" : "False";
  ...
  cmdGeneratePreview: function cmdGeneratePreview () {
    ...
    // The upload channel
    var previewURL = Cx.PDF_PREVIEW_URL;
    // if (gScriptController.mode == "av")
    //   previewURL += "av";
  ...
  try {
    ...
    try {
      ...
      if (node && node.parentNode) {
        ...
        cursorspan = gScriptController.editor.editor.createElementWithDefaults("span");
        cursorspan.appendChild(gScriptController.editor.contentDocument.createTextNode(""));
    ...
    dump("--- serialized PDF pre-filter to " + tmpFile.path + "\n");
