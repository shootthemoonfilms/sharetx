var gCardController = {
  ...
  loaded: function loaded () {
    ...
    for (var i = 0; i < this.colours.length; ++i)
      this.tagnames[this.colours[i]] = getPrefString(ps, this.colours[i]);
  },
  ...
  open: function open (project, docres) {
    ...
    if (tagnames) {
      ...
      if (tagnames.length != this.colours.length) {
        dump("*** gCardController.open: incorrect number of tag names");
        return;
      }
  ...
  save: function save () {
    ...
    while (! tagnames.isEmpty())
      tagnames.remove(0);
    for (var i = 0; i < this.colours.length; ++i) {
      var name = rdfsvc.GetLiteral(this.tagnames[this.colours[i]]);
      tagnames.push(name);
    }
    ds.endUpdateBatch();
  },
  ...
  onMouseMove: function onMouseMove (event) {
    ...
    // this._dragElement.setAttribute("style",
    //   "opacity: 0.5; z-index: 1; position: absolute; left: " + x
    //     + "px; top: " + y + "px;");
  ...
  createScene: function createScene () {
    ...
    // setTimeout("gCardController.scrollToCard('" + scene.id + "')", 100);
  },
...
function cards_onEnterPrintPreview () {
  ...
  try {
    ...
    dump("*** cards_onEnterPrintPreview: " + ex + "\n");
  }
  */
...
function cards_onExitPrintPreview () {
  ...
  try {
    ...
    dump("*** cards_onExitPrintPreview: " + ex + "\n");
  }
  */
