...
var gListener = {
  handleEvent: function (event) {
    if (event.type == "DOMAttrModified") {
      setTimeout("gListener.addWindowListener()", 0);
      return;
    }

    if (event.type != "cxws_exec")
      return;
    switch (dialog.frame.contentWindow.cxws_op) {
      case "close":
        dialog.frame.contentWindow.removeEventListener("cxws_exec",
          this, false);
        if (dialog.frame.contentWindow.cxws_arg1)
          dialog.cfg.closeURL = dialog.frame.contentWindow.cxws_arg1;
        window.close();
        break;
      case "upload":
        ...
        prepareModel();
        dialog.cfg.syncURL = dialog.frame.contentWindow.cxws_arg1;
        sendModel();
        break;
    }
  },
  ...
  addWindowListener: function () {
    if (dialog.frame.docShell.busyFlags) {
      setTimeout("gListener.addWindowListener()", 100);
      return;
    }
    dialog.frame.contentWindow.addEventListener("cxws_exec", this, false);
  }
};


function loaded () {
  dialog.mode = window.arguments[0];
  dialog.cfg = window.arguments[1];

  dialog.self  = document.documentElement;
  dialog.close = dialog.self.getButton('cancel');

  dialog.deck = document.getElementById("deck");
  dialog.frame = document.getElementById("uploadframe");
  ...
  dialog.frame.addEventListener("DOMAttrModified", gListener, false);

  try {
    init();
    if (dialog.mode == 'sync') {
      var url = Cx.PRESYNC_URL + dialog.cfg.projectID + "/"
        + dialog.cfg.revision;
      dialog.frame.setAttribute("src", url);
...
function sendModel () {
  ...
  req.open('POST', dialog.cfg.syncURL);
  req.send(dom);
...
function fetchContents () {
  dialog.projRes = RES(dialog.cfg.projURL);
  ...
  req.open('GET', dialog.cfg.fetchURL);
  req.send(null);
...
function handleFinishResponse (evt) {
  ...
  var m = stringToModel(req.responseText);
  dump("--- handleFinishResponse: "+ req.responseText + "\n");
  ...
  if (dialog.mode == 'sync') {
    ...
    var postsyncURL = m.target(dialog.transRes, PROP('cx:postsync')).value;
    dialog.close.hidden = true;
    dialog.deadman = setTimeout(checkStatus, 300*1000);
    setTimeout(showServerPanelCallback, 100);
    dialog.frame.setAttribute("src", postsyncURL);
  }
