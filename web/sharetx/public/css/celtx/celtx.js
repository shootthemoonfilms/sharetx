...
function loaded () {
  ...
  if (window.arguments && window.arguments[0]) {
    if (window.arguments[0].match(/^celtxs?:\/\//))
      setTimeout(function () { loadCeltxURI(window.arguments[0]); }, 0);
    else
      setTimeout(function () { loadProject(window.arguments[0]); }, 0);
  }
  else {
...
function loadCeltxURI (uri) {
  try {
    ...
    tmpfile.append(unescape(uri.replace(/.*\//, "")));
...
function loadProject (projuri, isremote, wsref) {
  ...
  try {
    ...
    // Add the Media Manager
    try {
      gMediaManager = new MediaManager(gProject);
    }
    catch (ex) {
      dump("*** new MediaManager: " + ex + "\n");
    }
    ...
    try {
      ...
      if (saveInterval > 0)
        gWindow.autosaveTimer = setInterval("autosaveProject()",
...
function updateExternalFileIcons () {
  ...
  for (var i = 0; i < extdocs.length; i++) {
    ...
    dump("--- updating an external file icon\n");
...
var gFrameLoader = {
  ...
  updateMenuCommands: function updateMenuCommands () {
    ...
    if (! frame || ! frame.menupopup)
      return;
    var menuitems = frame.menupopup.getElementsByTagName("menuitem");
    for (var i = 0; i < menuitems.length; ++i) {
      if (menuitems[i].hasAttribute("command"))
        goUpdateCommand(menuitems[i].getAttribute("command"));
    }
  },
  ...
  addMenu: function addMenu (frame) {
    ...
    var menuitems = frame.menupopup.getElementsByTagName("menuitem");
    for (var i = 0; i < menuitems.length; i++) {
      var command = menuitems[i].getAttribute("command");
      if (command) {
        command = frame.id + "_" + command;
        menuitems[i].setAttribute("command", command);
      }
    }
  },
  ...
  onTabClose: function onTabClose (event) {
    ...
    if (this.currentFrame &&
      ...
      dump("-- Fixing focus due to background close\n");
  ...
  closeFrame: function closeFrame (frame) {
    ...
    // var tabContainer = gWindow.documentTabbox.firstChild.firstChild;
    // var panelContainer = gWindow.documentTabbox.lastChild;
    var index = -1;
    for (var i = 0; i < this.frames.length; i++) {
    ...
    if (index < 0)
      throw "Frame is not in frame list";
    ...
    var currentIndex = gWindow.documentTabbox.selectedIndex;
    var newIndex = -1;
    ...
    if (this.frames.length <= 1) {
      gWindow.documentTabbox.setStripVisibilityTo(false);
      // Removing a menu item is not implemented in nsMenuX.cpp, so this
      // doesn't work on the Mac... why not? I don't know.
      document.getElementById("menu_closeWindow").hidden = true;
      var menuClose = document.getElementById("menu_close");
      menuClose.setAttribute("label", gApp.getText("Close"));
    }

    if (this.frames.length > 0) {
      var newFrame = this.frames[newIndex];
      newFrame.tab.selected = true;
      panelContainer.selectedPanel = newFrame.panel;
      // gWindow.outlineDeck.selectedPanel = newFrame.outline;
      setTimeout("gWindow.outlineDeck.selectedPanel = "
        + "gFrameLoader.currentFrame.outline;", 0);
      // this.focusFrame(newFrame);
    }
  ...
  loadNextDocument: function loadNextDocument () {
    ...
    */
    ...
    try {
      ...
      var tabs = gWindow.documentTabbox.firstChild.firstChild;
      var tabpanels = gWindow.documentTabbox.lastChild;
      docframe.panel = document.createElement("iframe");
      tabpanels.appendChild(docframe.panel);
      docframe.panel.setAttribute("id", docframe.id);
      docframe.panel.addEventListener("celtxload", gFrameLoader, false);
      docframe.panel.setAttribute("src", viewer.Value);
      dump("*** loading " + docframe.id + "\n");

      docframe.outline = document.createElement("iframe");
      gWindow.outlineDeck.appendChild(docframe.outline);
      docframe.outline.addEventListener("celtxload", gFrameLoader, false);
      dump("outline is " + outline.Value + "\n");
      docframe.outline.setAttribute("src", outline.Value);

      docframe.tab = tabs.appendItem(title, docframe.id);
      if (thumbnail)
        docframe.tab.setAttribute("image", thumbnail.Value);
      docframe.tab.setAttribute("crop", "end");
      docframe.tab.setAttribute("class", "celtx-tab");
      docframe.tab.setAttribute("flex", 100);
      gWindow.documentTabbox.selectedTab = docframe.tab;
      */
...
var gDocDNDObserver = {
  ...
  attemptExternalDrop: function attemptExternalDrop (row, orientation,
    ...
    var rowres = this.tree.view.getResourceAtIndex(row);
    var dropindex = -1;
    var seq = null;
    if (orientation == this.DROP_ON && this.tree.view.isContainer(row)) {
      seq = new RDFSeq(gProject.ds, rowres);
    }
    else {
      // Get the row's parent
      var rowparent = this.tree.view.getParentIndex(row);
      if (rowparent < 0)
        rowparent = gProject.rootFolder.res;
      else
        rowparent = this.tree.view.getResourceAtIndex(rowparent);
      seq = new RDFSeq(gProject.ds, rowparent);
      dropindex = seq.indexOf(rowres);
    ...
    try {
      ...
      if (dropindex >= seq.length)
        seq.push(newres);
      else
        seq.insert(newres, dropindex);
    }
    catch (ex) {
  ...
  addItemsToTree: function (dragSession, row, orientation) {
    // The target sequence to put the items in
    var seq = null;
    // Starting offset in the target sequence, not a tree row offset
    var offset = 0;
    if (orientation == this.DROP_ON && this.tree.view.isContainer(row)) {
      var res = this.tree.view.getResourceAtIndex(row);
      seq = new RDFSeq(gProject.ds, res);
      offset = seq.length;
    }
    else {
      var parentrow = this.tree.view.getParentIndex(row);
      var rowlevel = this.tree.view.getLevel(row);
      var cursor = parentrow + 1;
      while (cursor <= row) {
        if (this.tree.view.getLevel(cursor) == rowlevel)
          ++offset;
        ++cursor;
      }
      if (orientation == this.DROP_AFTER)
        ++offset;
      var res = this.tree.view.getResourceAtIndex(parentrow);
      seq = new RDFSeq(gProject.ds, res);
    }
    ...
    for (var i = 0; i < dragSession.numDropItems; ++i) {
      try {
        ...
        if (gCatalogManager.isBreakdownItem(itemres)) {
          ...
          if (parentres.EqualsNode(seq.res)) {
  ...
  onDrop: function onDrop (row, orientation) {
    ...
    // canDrop is called before onDrop
    /*
    if (dragSession.numDropItems != 1 ||
        ! dragSession.isDataFlavorSupported("moz/rdfitem")) {
      dump("*** Multiple items, or no moz/rdfitem or text/unicode flavour\n");
      return;
    }
    */
    ...
    var seq = null;
    if (orientation == this.DROP_ON && this.tree.view.isContainer(row)) {
      // Can't add a folder to itself
      if (isFolderAncestorOf(res, rowres)) {
      ...
      if (rowparent < 0)
        ...
        rowparent = this.tree.view.getResourceAtIndex(rowparent);
      // Can't add a folder to itself
      if (isFolderAncestorOf(res, rowparent)) {
        dump("--- can't add a folder to itself\n");
        return;
      }
      seq = new RDFSeq(gProject.ds, rowparent);
    ...
    if (parentseq.res.Value == seq.res.Value) {
    ...
    try {
      ...
      if (dropindex < 0 || dropindex >= seq.length) {
        seq.push(res);
      }
      else {
        seq.insert(res, dropindex);
      }
    }
    catch (ex) {
...
function projectWindowWillClose () {
  ...
  try {
    dump("--- removing celtx.js::gController\n");
...
function saveProjectToWorkspace () {
  ...
  var observer = {
    onLogin: function (result) {
      ...
      openDialog(Cx.CONTENT_PATH + "sync2.xul", "", Cx.MODAL_DIALOG_FLAGS, cfg);
    }
  };
...
function isProjectModified () {
  ...
  var savefile = getProjectFile();
  if (savefile) {
    try {
      // The following three lines make no sense.
      if (savefile.exists())
        return false;
      var lastmod = savefile.lastModifiedTime;
      var rdfsvc = getRDFService();
      var doctypearc = rdfsvc.GetResource(Cx.NS_CX + "doctype");
      var doctype = rdfsvc.GetResource(Cx.NS_CX + "ExternalDocument");
      var extdocs = gProject.ds.GetSources(doctypearc, doctype, true);
      while (extdocs.hasMoreElements()) {
        var extdoc = extdocs.getNext().QueryInterface(
          Components.interfaces.nsIRDFResource);
        var extfile = gProject.localFileFor(extdoc);
        if (extfile && extfile.lastModifiedTime >= lastmod)
          return true;
      }
    }
    catch (ex) {
      dump("*** isProjectModified: " + ex + "\n");
    }
  }
...
function closeCurrentTab () {
  ...
  var modified = false;
  try {
    modified = frame.controller && frame.controller.modified;
  }
  catch (ex) {
    dump("*** frame.modified: " + ex + "\n");
  }
    ...
    if (config.type.Value == Cx.NS_CX + "ExternalDocument") {
      ...
      if (config.type.Value == Cx.NS_CX + "StoryboardDocument2") {
        ...
        if (config.source) {
          ...
          storyboard.appendChild(<sequence title={untitled}/>);
        }
...
function cmdProjectProperties () {
  var config = gProject.publishSettings;
  config.accepted     = false;
  config.title        = gProject.title;
  config.tagline      = gProject.tagline;
  config.description  = gProject.description;
  config.language     = gProject.language;
  config.location     = gProject.projectFolder;
  config.modified     = gProject.modified;
  config.image        = gProject.image;
  config.embed        = gProject.embed;
  config.authURL      = Cx.AUTH_URL;
  config.publishURL   = Cx.PUBLISH_URL;
  window.openDialog(Cx.CONTENT_PATH + "projectprops.xul", "_blank",
                    Cx.MODAL_DIALOG_FLAGS, config);
  if (! config.accepted)
    return;
  gProject.title        = config.title;
  gProject.tagline      = config.tagline;
  gProject.description  = config.description;
  gProject.language     = config.language;
  gProject.image        = config.image;
  gProject.embed        = config.embed;

  gProject.savePublishSettings(config);
  saveProject();
}
...
function updateNotifierLabel () {
  ...
  if (! frame.docShell || frame.docShell.busyFlags) {
    setTimeout(updateNotifierLabel, 500);
    return;
  }
  ...
  function checkForRedirect () {
    ...
    try {
      ...
      if (channel.responseStatus < 200 || channel.responseStatus >= 300)
        throw new Error("Unable to load notifier");
      ...
      dump("*** updateNotifierLabel: " + ex + "\n");
     }
  }
  setTimeout(checkForRedirect, 0);
}
